...
def make_play(play, board):
    ...
    ###Your code here.
    ### the board will now have more letters on it, also update exising letters
    # board is a list of rows which is a list of sqs
    for n, L in enumerate(word):
        board[i+n*di][j+n*dj] = L
        # mutate each single character, not the whole string
        # enumerate is a great function to use!!!!
    '''if play[2] == ACROSS:
        row = board[i]
        newrow = row[:j]+word+row[j+len(word):]
        board[i]= newrow
        # board[i][j:j+len(word)] = word #'str' object does not support item assignment
    else:
        vboard = transpose(board)
        row = board[j]
        newrow = row[:i]+word+row[i+len(word):]
        board[j]= newrow
        board = transpose(vboard)'''
    return board
