...
(For the example word w='adole'+'scent'+'ed', the
...
def natalie(words):
    ...
    # suffix is prefixset, which is mid
    results = set() # set ((start,mid,end),(),())
    for word1 in words:
        for word2 in words:
            if word2 != word1:
                for mid in suffix(word1): # mid is not ''
                    if mid in prefix(word2):
                        word1r = word1[::-1]
                        midr = mid[::-1]
                        startr= word1r[len(midr):] # solve the first appearance repeat issue, just reverse and do the same for prefix
                        start = startr[::-1]
                        end = word2[len(mid):]
                        results.add((start,mid,end))
    if results:
        best_combo = max(results, key = scores)
        return ''.join(best_combo)
    else:
        return None

def scores(portmanteau):
    start,mid,end = portmanteau # each word is a tuple, not a string, then join the string later
    
    total = len(start)+len(mid)+len(end)
    
    starti = endi = total/4 # rounding number
    midi = total/2
    
    score = total - abs(starti-len(start)) - abs(midi-len(mid)) - abs(endi-len(end))
    return score
...
# print prefix('love')
# set(['', 'lo', 'lov', 'l'])
# print suffix ('love'), set(['e', 've', 'ove'])
...
def test_natalie():
    ...
    # assert natalie(['eskimo', 'escort', 'kimchee', 'kimono', 'cheese']) == 'eskimono' #kimcheese
