...
def boggle_words(board, minlength=3):
    ...
    # your code here
    # board is a string of size (N+2)**2 with borders all around
    N = size(board) # N, actual letter is (N-2)**2
    results = set()

    # record path in list of locations, because not to repeat, instead of change the board to border
...
def find_boggle(board,i,w,minlength): # find words starting at i positions in the board
    results = set() 
    N = size(board)
    nw = w + board[i]
    if nw in WORDS and len(nw) >= minlength: results.add(nw)
    if nw in PREFIXES: # can't be border then
        newboard = board.replace(board[i],'|',1)
        for j in neighbors(i, N): #(i-N-1, i-N, i-N+1, i-1, i+1, i+N-1, i+N, i+N+1)
            find_boggle(newboard,j,nw,minlength)
    return results
