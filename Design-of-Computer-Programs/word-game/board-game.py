...
def horizontal_plays(hand, board):
    ...
    for (j, row) in enumerate(board[1:-1], 1):
        ...
        for (i, word) in row_plays(hand, row):
            score = calculate_score(board, (i, j), ACROSS, hand, word) # HERE SCORE IS ALREADY DEFINED AS TOTAL SCORES
...
def best_play(hand, board):
    ...
    ###Your code here.
    # calculate_score(board, pos, direction, hand, word)
    
    allplays = all_plays(hand,board) # it is a set, can't index
    if allplays:
        return max(allplays) # since score is the first element, directly call max, otherwise, need a key
    else:
        return NOPLAY
