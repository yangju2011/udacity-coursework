...
def boggle_words(board, minlength=3):
    ...
    def extend_boggle(pre,path):
    # previous words = pre, path = [i,j,d...]
        i = path[-1]
        if pre in WORDS and len(pre) >= minlength: results.add(pre)
        if pre in PREFIXES:
            for j in neighbors(i,N):
                if j not in path and j != "|": #make sure j is not border
                    extend_boggle(pre+board[j],path+[j])        
    
    for (i,L) in enumerate(board):
        if L != "|":
            extend_boggle(L,[i]) #does not need to return, because extend it self add to results
    return results
