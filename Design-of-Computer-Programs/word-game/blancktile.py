...
# isupper(),check if it is actual tile, islower(), check if it is just a 0 score letter
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if pre in PREFIXES:       # all caps, but here in pre+l it has lower case
        sq = row[i]
        if is_letter(sq):
            ...
            if '_' in hand: # allow blanks
                for C in possibilities: # add on the lowercase
                    add_suffixes(hand.replace(L, '', 1), pre+C.lower(), start, row, results)
    return results
...
def find_prefixes(hand, pre='', results=None):
    ...
    PRE= pre.upper() # because the word has both cap and lower
        ...
        for L in hand:
            if L == '_':
                for C in LOWER_LETTERS:
                    find_prefixes(hand.replace(L, '', 1), pre+C, results)
            else:
