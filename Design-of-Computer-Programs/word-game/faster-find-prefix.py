...
'''def find_prefixes(hand):    
    ...
    global prev_hand, prev_results # need the global claim
    
    if prev_hand == hand: return prev_results
    else:
        results = find_prefixes2(hand, pre='', results=None)
        prev_hand = hand
...
def find_prefixes2(hand, pre='', results=None):
    ...
    if pre in WORDS or pre in PREFIXES: results.add(pre) # pre could also be in PREFIXES by itself
    if pre in PREFIXES:
        for L in hand:
            find_prefixes2(hand.replace(L, '', 1), pre+L, results)
    return results'''


def find_prefixes(hand, pre='', results=None):
    """Find all prefixes (of words) that can be made from letters in hand."""
    global prev_hand, prev_results  # need to add global !!!
