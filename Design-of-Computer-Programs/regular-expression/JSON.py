...
def grammar(description, whitespace=r'\s*'):
    ...
    # pass  value, which is string, number, object, array, true, ArithmeticError, false, null
    # object --> {},{ members }
    # members --> pair or pair , members
    # pair --> string:value
    # array -->[] or [element]
    # element --> value or value element

    
    G = {' ': whitespace}
    description = description.replace('\t', ' ') # no tabs!
    # since re is already imported, \t is recognized
    for line in split(description, '\n'): 
        lhs, rhs = split(line, ' => ', 1) # need more than 1 value to unpack
...
# return the longer element first, pair, member | pair
# no need to add additiona [] around {} and , :
JSON = grammar(
"""object    =>   { } | { members } 
...
int       =>   -?[1-9][0-9]*
frac      =>   [.][0-9]+
exp       =>   [eE][-+]?[0-9]+ """, whitespace='\s*') # need more than 1 value to unpack
# ? is not certain
# string is any char
# 
...
print json_parse('["testing", 1, 2, 3]') # None, None

def test():
    assert json_parse('["testing", 1, 2, 3]') == (                      
                       ['value', ['array', '[', ['elements', ['value', 
                       ['string', '"testing"']], ',', ['elements', ['value', ['number',['int', '1']]], ',', ['elements', ['value', ['number',['int', '2']]], ',', ['elements', ['value', ['number',['int', '3']]]]]]], ']']], '')
