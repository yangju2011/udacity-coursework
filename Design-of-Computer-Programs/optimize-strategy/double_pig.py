...
def pig_actions_d(state):
    ...
    if double == 'double':
        ...
        return ['double','roll','hold'] if pending else ['double','roll']
    elif double > 1:
        return ['roll','hold'] if pending else ['roll']

def strategy_d(state): # maximize the Pwin of the good amount, choose action for each state
    (p, me, you, pending, double) = state
    return 'double' if (double == 1 and me+pending+1>=goal) else hold_20_d(state)
    # your code here
...
def strategy_d(state): # maximize the Pwin of the good amount, choose action for each state
    # your code here
    "Hold at 20 pending.  Always accept when winning. Never double. In 1000 games of pig, strategy strategy_d took 52.6 percent of the points against hold_20_d."
    "Hold at 40 pending.  Always accept when winning. Never double. In 1000 games of pig, strategy strategy_d took 48.9 percent of the points against hold_20_d."
    "Hold at 20 pending.  Always accept. Double when wininning >20. In 1000 games of pig, strategy strategy_d took 57.4 percent of the points against hold_20_d. "
    "Hold at 20 pending.  Always accept. Double when wininning. In 1000 games of pig, strategy strategy_d took 58.48 percent of the points against hold_20_d. "
    "Hold at 20 pending.  Accept within 10 points Decline when within 10 points. Double when wininning. In 1000 games of pig, strategy strategy_d took 57.6271186441 percent of the points against hold_20_d."
    "Hold at 20 pending.  Accept when wining Decline >= 20 points. Double when wininning. In 1000 games of pig, strategy strategy_d took 59.68 percent of the points against hold_20_d."
    "Hold at 20 pending.  Accept when wining Decline >= 30 points. Double when wininning. In 1000 games of pig, strategy strategy_d took 62.34 percent of the points against hold_20_d."
    # but it is not certain, sometimes is is >60%
    return best_action(state, pig_actions_d, Q_pig_d, Pwin)

def Q_pig_d(state,action,Pwin): # expected award of winning 
    (p, me, you, pending, double) = state
    if action not in pig_actions_d(state): # not legal, opponent wins
        return 0
    elif action == 'roll':
        return double*((1-Pwin((other[p], you, me+1, 0, double)) + sum(Pwin((p, me, you, pending+d, double)) for d in (2,3,4,5,6)))/6.)
    elif action == 'hold':
        return double*(1-Pwin((other[p], you, me+pending, 0, double)))
    elif action == 'double':
        return 2*(1-Pwin((other[p], you, me, pending, 'double'))
    elif action == 'decline': # decline then opponent wins
        return 0
    elif action == 'accept':
        return 2*(1-Pwin((other[p], you, me, pending, 2)))


@memo
def Pwin(state): # utility, probabity of winning
    (p, me, you, pending, double) = state
    if me+pending >= goal:
        return 
    elif you >= goal:
        ...
        return max(Q_pig_d(state,action,Pwin) for action in pig_actions_d(state)) # Q is not probability, but Pwin is probability'''
