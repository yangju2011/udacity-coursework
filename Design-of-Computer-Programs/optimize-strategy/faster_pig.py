...
@memo
def Pwin3(me, you, pending): # don't need p, whichever player
   ## your code here
    if me + pending >= goal:
        return 1
    elif you >= goal:
        ...
        # action --> state
        # hold --> you, me+pending,0
        # roll --> if roll = 1, you, me+1, 0; else me, you, pending+d
        # Pwin3 needs to maximize the Pwin of me, and minimize the Pwin of you
        
        # if pending = 0, have to roll
        # could use Proll to represent the very long 1-Pwin3(you,me+1,0)+sum((Pwin3(me,you,pending+d) for d in (2,3,4,5,6)))/6.

        # max[Q(state,action,U) for action in actions]
        Proll = (1-Pwin3(you,me+1,0) + sum((Pwin3(me,you,pending+d) for d in (2,3,4,5,6))))/6.
        # 6. is for all conditions, not just the sum
