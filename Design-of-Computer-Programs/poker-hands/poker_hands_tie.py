...
def allmax(iterable,key = None):
    # write max function yourself
    # key is a function they can take input, None is the value of the function key
    maxlist = []
    maxval = None #intial value of maxval does not matter, but should be None, any number is bigger than None
    
    xval = key or (lambda x: x)
    # if key is None, return lambda x:x
    ''' same as
    if key != None:
        xval = key
    else:
        xval = lambda x:x
        # xval = x is not a function for undefined x'''
    # key = key or (lambda x:x) then xval = key(x)
    
    # xval is self , xval is a function, which returns x for input x
    for x in iterable:
        if xval(x) > maxval:
            maxval = xval(x)
    # find the maxval first
...
def test():
    ...
    # handle ties
    tsf = ['7C','8C','5C','4C','6C']
    # it should return both sf and tsf
    assert poker([tsf,sf]) == [tsf,sf]
    assert poker([sf,tsf]) == [sf,tsf]
    # whoever comes first will be returned, however, both should win
    # therefore, instead of return the max hand, it should return a list of max hands
