...
# A->B a reciprocal link if there   The length of a link path 
...
def compute_ranks(graph,k):
    ...
    npages = len(graph)
    for page in graph:
        ranks[page] = 1.0/npages #rank is all the same in the beginning, start number not change, assume nothing is linked
    for i in range(0, numloops):
        newranks = {}
        for page in graph:
            newrank = (1 - d) / npages
            cpages=[page] #collusion page is self when k=0
            i=0
            while i<k:
                for cp in cpages:
                    cpages=cpages+graph[cp] #add the child outlinks of page to collusion page list
                i=i+1
            for node in graph:
                if page in graph[node] and node not in cpages:
